    ...
    <Box sx={{ height: "100%" }}>
      ...
      >
        {widgetA?.map((widget, index) => {
          ...
          if (widget.transform_data.length > 0) {
            if (widget.transform_data[0]?.dataTable && widget.transform_data[0]?.dataTable.length > 0) {
              keysArry = Object.keys(widget.transform_data[0].dataTable[0]);
            }
          } else if (widget.data[0]?.dataTable && widget.data[0]?.dataTable.length > 0) {
            keysArry = Object.keys(widget.data[0].dataTable[0]);
          }
          // if (widget.data[0]?.dataTable && widget.data[0]?.dataTable.length > 0) { // 這邊暫時預設data[0] 之後可能會改成selected query
          //   keysArry = Object.keys(widget.data[0].dataTable[0]);
          // }
                ...
                <>
                  ...
                  >
                    <GraphTypeSwitcher
                      ...
                      data={widget.transform_data.length > 0 ? widget.transform_data[0].dataTable : widget.data[0].dataTable} ///// 這邊data[0]暫時預設是第0個data 之後可能會改成selected query
                      // data={widget.data[0].dataTable} ///// 這邊data[0]暫時預設是第0個data 之後可能會改成selected query
                      // width={500}
                      // height={300}
